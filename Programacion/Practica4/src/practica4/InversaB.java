 ...
 * date 2023-03-26
 */
public class InversaB {
    public static void ejecutar(int[][] matrizB) {
        if (matrizB.length != 3 || matrizB[0].length != 3) {
            System.out.println("Error: la matriz B debe ser de 3x3 para calcular su inversa");
            return;
        }

        int det = calcularDeterminante(matrizB);
        if (det == 0) {
            System.out.println("Error: la matriz B no tiene inversa porque su determinante es cero");
            return;
        }

        int[][] adjunta = calcularAdjunta(matrizB);
